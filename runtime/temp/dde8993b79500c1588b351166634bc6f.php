<?php /*a:4:{s:60:"F:\WWW\M\application\shop\view\sys_admin\headline\index.html";i:1613985989;s:53:"F:\WWW\M\application\mainadmin\view\layouts\base.html";i:1613985988;s:59:"F:\WWW\M\application\shop\view\sys_admin\headline\list.html";i:1613985989;s:53:"F:\WWW\M\application\mainadmin\view\layouts\page.html";i:1613985988;}*/ ?>
 ...
 <form id="forms" class="talbe-search form-inline "  method="post" action="<?php echo url('getList'); ?>" >
     <div class="page-breadcrumbs">
            <ul class="breadcrumb" >
             ...
             <input type="text" class="form-control input-large" value="<?php echo htmlentities($search['keyword']); ?>" name="keyword" placeholder="头条标题" data-rule-required="true" />
             <button class="btn btn-sm btn-default-iq" type="submit" title="搜索"><i class="fa fa-search"></i></button>
             <a href="<?php echo url('info'); ?>" class="btn btn-sm btn-default fr m-r-tm m-t-md"><i class="fa fa-plus"></i>添加头条</a>
        </div>
    </form>
</header>
...
<thead >
 <thead>
        <tr>
           <th width="50" class="th-sortable" data-sort-name="id">ID</th>
            <th>头条标题</th>
            <th>所属分类</th>
            <th>点击量</th>
            <th>发布时间</th>
            <th>修改时间</th>
            <th width="100" style="text-align:center;">首页推荐</th>
            <th width="100" style="text-align:center;">显示状态</th>
            <th>排序</th>
...
<?php if(is_array($data['list']) || $data['list'] instanceof \think\Collection || $data['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
 <tr>
     ...
     <td align="center"><a href="#" data-remote="<?php echo url('ajaxEdit',array('id'=>$vo['id'],'field'=>'is_best')); ?>" class="<?php echo htmlentities(tplckval($vo['is_best'],'=1','active')); ?>"  data-toggle="class" data-ajax="true"><i class="fa fa-check text-success text-active">是</i><i class="fa fa-times text-danger text">否</i></a></td>
     <td align="center"><a href="#" data-remote="<?php echo url('ajaxEdit',array('id'=>$vo['id'],'field'=>'is_show')); ?>" class="<?php echo htmlentities(tplckval($vo['is_show'],'=1','active')); ?>"  data-toggle="class" data-ajax="true"><i class="fa fa-check text-success text-active">显示</i><i class="fa fa-times text-danger text">隐藏</i></a></td>
     <td><?php echo htmlentities($vo['sort_order']); ?></td>
     <td>
         <a href="<?php echo url('info',array('id'=>$vo['id'])); ?>" class="m-xs" title="编辑"><i class="fa fa-edit text-muted"></i></a>
         <a href="<?php echo url('del',array('id'=>$vo['id'])); ?>" data-toggle="ajaxRemove" data-msg="确定删除 <?php echo htmlentities($vo['title']); ?>？" class="m-xs" title="删除"><i class="fa fa-trash text-muted"></i></a>
     </td>
 </tr>

...
var _uploadUrl='/publics/api.index/upImage' //异步上传图片的接口路径
...
function compress(event, callback) {
	if ( typeof (FileReader) === 'undefined') {
		...
		try {
			...
			//调用上传方式  不压缩
			asyncUpload(e.target.result,callback);
		}
	}
}
...
function AppReturnBase64Image(image) {
    if(typeof(_uplocalfunction) == 'function'){
    	asyncUpload(image,_uplocalfunction)
    }else{
        _alert('未定义方法')
    }
...
//异步上传图片
function asyncUpload(file,callback){

...
class ClientbaseController extends BaseController{
    ...
    public function initialize()
    {
        ...
        if ($this->is_wx == 1) {
            $ShareTemp = (new \app\distribution\model\ShareTempModel)->where('status',1)->order('id DESC')->find();
            $wxShare = (new \app\weixin\model\WeiXinModel)->getSignPackage();
            $wxShare['img'] = $ShareTemp['images'];
            $wxShare['title'] = $ShareTemp['title'];
            $wxShare['description'] = $ShareTemp['describe'];
            $DividendShareByRole = settings('DividendShareByRole');
            if ($DividendShareByRole == 0 || $this->userInfo['role_id'] > 0){
                $wxShare['shareUrl'] = getUrl();
            }
    ...
    public function logout()
    {
        ...
        (new \app\member\model\UsersModel)->logout();
        if ($this->request->isAjax()){

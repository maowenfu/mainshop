...
class UsersModel extends BaseModel
{
    ...
    public function login($data = array())
    {
        ...
        $userInfo = $this->where('nick_name', $nick_name)->find();
        if (empty($userInfo)) {
    ...
    //-- 系统登陆后执行
    /*------------------------------------------------------ */
    public function doLogin($user_id, $source = '',$model = 'shop')
    {
    ...
    public function register($inArr = array(), $wxuid = 0, $is_admin = false, &$obj = '')
    {
        ...
        if ($wxuid == 0) {
            ...
            if (empty($inArr['nick_name']) == false) {//用户名不为空时，判断是否已存在
            ...
            if ($count > 0) return '用户名：' . $inArr['nick_name'] . '，已存在.';
    ...
    public function info($val, $type = 'user_id', $isCache = true)
    {
        ...
        $info['region_text'] = '';
        if ($info['district'] > 0){
            $region = (new \app\mainadmin\model\RegionModel)->info($info['district']);

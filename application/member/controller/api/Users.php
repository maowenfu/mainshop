...
class Users extends ApiController
{
    ...
    {
        $sms_fun = settings('sms_fun');//获取短信配置
        ...
        if (empty($old_password) ){
            ...
            $pay_password1 = $this->Model->where('user_id',$this->userInfo['user_id'])->value('pay_password');
            $old_password = f_hash($old_password);
            if ($old_password != $pay_password1){
    ...
    {
        $res = $this->checkCode('bind_mobile', input('mobile'), input('code'));//验证短信验证
        if ($res !== true){
            return $this->error($res);
        }
    ...
    {
        $upArr['headimgurl'] = input('imgfile');
    ...
    public function dowaterimg($bgimage,$qrcode,$pathname,$type,$header_img,$user){
        ...
        $image->text("用户名 ".$user['nick_name'], "hgzb.ttf", 30,"#000",$location3)->save($pathname);

...
function exportExcel($data = array() , $exportData = array() , $ColumnDimension = array(),$fileName = '订单列表_',$fileType = "xlsx")
{
    ...
    foreach ($data as $key => $value) {
        ...
        foreach ($exportData as $k => $name) {
            if (is_numeric($value[$k]) == true) {
                $obj->getActiveSheet()->setCellValue($list[$z]. ($key + 2), $value[$k], \PHPExcel_Cell_DataType::TYPE_STRING);
            }else{
    ...
    // 设置第一列为纯文本
    $obj->getActiveSheet()->getStyle('A1')->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_NUMBER);
    // 设置列宽
...
function getFileExtend($file){
    if (strstr($file,'http://') == true || strstr($file,'https://') == true){//网络图片
...
function asynRun($rule,$param){
    ...
    if (function_exists('lzasyn')){//建议使用此方法，需加载lzasyn.so扩展
        $url = getcwd().'/asynRun.php';
        $query = str_replace('&',' ',$query);
        echo $url.' '.$query.' &';
        $pid = lzasyn($url.' '.$query.' &','r');
        //var_dump(fread($pid,500)) ;
        pclose($pid);
        return true;
    }

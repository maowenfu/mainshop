...
class OrderModel extends BaseModel
{
    ...
    function upInfo($upData, $extType = '')
    {
        ...
        if ($upData['order_status'] == $this->config['OS_CONFIRMED']) {//确认订单
            ...
            if ($orderInfo['order_type'] == 2) { // 拼团订单清除缓存
        ...
        if ($sendMsg == true){
                ...
                $sendData['address'] = $orderInfo['merger_name'] . $orderInfo['address'];
                $sendData['goods_name'] = $this->getPayBody($orderInfo['order_id']);//$orderInfo['merger_name'] . 

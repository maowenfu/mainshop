...
class GoodsModel extends BaseModel
{
    ...
    //-- 获取商品分类
    /*------------------------------------------------------ */
    public function getClassList($pid = false,$isJs = false)
    {
        $CategoryModel = new CategoryModel();
        return $CategoryModel->getRows($pid,$isJs);
    }
    ...
    public function getGoodsSku(&$goods)
    {
        ...
        $gsrows = $GoodsSkuModel->where('goods_id', $goods['goods_id'])->order('sku_val ASC')->select()->toArray();
        $skuarr = [];
        ...
        if (empty($this->userInfo) == false) {//计算会员价格
            ...
            if ($goods['level_price_type'] > 0) {
                if ($goods['level_price_type'] == 1) {//会员等级折扣
                    $all_price[] = priceFormat($all_price[0] * $this->userInfo['level']['level_pro'] / 100);
                } else {
                    ...
                    if ($u_price > 0) {
                        if ($goods['level_price_type'] == 2) {//自定义折扣
                            $all_price[] = priceFormat($all_price[0] * $u_price / 100);

...
class weixin
{
    ...
    function get_H5code($order, $config)
    {
        ...
        if ($return['return_code'] == 'SUCCESS' && $return['result_code'] == 'SUCCESS') {
            $back_url = url('shop/flow/done',array('order_id'=>$order['order_id']),true,true);
            return ['status' => 1, 'msg' => $return['return_msg'], 'result' => $return['mweb_url'].'&redirect_url='. urlencode($back_url)];
        } else {
